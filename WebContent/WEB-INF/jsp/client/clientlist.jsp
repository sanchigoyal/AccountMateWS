				...
				<table id="clientList" class="table">
					...
					<tbody>
						...
						<tr id="parentrow${client.clientID}">
							...
							<a data-toggle="modal" href="#editClient" 
								<c:choose>
									<c:when test="${client.clientCategoryName ne 'CASH'}">
										class="addLineSeperator"
									</c:when>
									<c:otherwise>
										class="addLineSeperator not-active"
									</c:otherwise>
								</c:choose>
							    onClick="updateEditModal('${client.clientID}');" title="Edit"><i class="fa fa-edit fa-lg extraPaddingLeftRight5"></i></a>
							<a data-toggle="modal" href="#clientDelete" 
								<c:if test="${client.clientCategoryName eq 'CASH'}">
									class="not-active"
								</c:if>
							onclick="updateDeleteModal('${client.clientName}','${client.clientID}','${client.clientBalance}');" data-toggle="tooltip" data-placement="bottom" title="Delete"><i class="fa fa-trash fa-lg extraPaddingLeftRight5"></i></a></p></td>
						</tr>
